...
-filter_complex "
[0:v]format=yuv420p,crop=min(iw\,ih):min(iw\,ih),scale=600:600,fade=t=in:st=0:d=1,fade=t=out:st=$VIDEO_FADE_OUT:d=1[cover];
[0:v]format=yuv420p,crop=min(iw\,ih):min(iw\,ih),scale=1920:1080:force_original_aspect_ratio=increase,gblur=sigma=25,scale=2000:1125,crop=1920:1080,rotate='0.03*sin(2*PI*t/6)':fillcolor=black@0,fade=t=in:st=0:d=1,fade=t=out:st=$VIDEO_FADE_OUT:d=1[bg];
[bg][cover]overlay=(W-w)/2:(H-h)/2[vbase];
[2:v]scale=140:-1,fade=t=in:st=5:d=0.5:alpha=1,fade=t=out:st=10:d=0.5:alpha=1[logo];
[vbase][logo]overlay=30:30:enable='between(t,5,10)'[v];
[1:a]afade=t=in:st=0:d=1,afade=t=out:st=$AUDIO_FADE_OUT:d=1[a]
" \
-map "[v]" \
-map "[a]" \
